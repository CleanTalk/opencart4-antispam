...
class SFW
{
    ...
    public function __construct($db_object, $db_prefix)
    {
        ...
        $this->ip_array = $this->ipGet(array('real'), true);
    }
    ...
    public function run($apikey)
    {
        ...
        if ( $is_sfw_check ) {
            ...
            if ( $this->pass === false ) {
                foreach ( $this->blocked_ips as $ip ) {
                    $this->logsUpdate($ip['ip'], 'blocked');
                }
                $this->sfwDie($apikey, '', Server::get('HTTP_HOST'));
            } else {
                reset($this->passed_ips);
                if ( !headers_sent() && key($this->passed_ips) ) {
                    Helper::apbctCookieSet(
                        'ct_sfw_pass_key',
                        md5($this->passed_ips[key($this->passed_ips)]['ip'] . $apikey),
                        time() + 86400 * 30,
                        '/',
                        null,
                        false
    ...
    private function ipGet(
        $ips_input = array('real', 'remote_addr', 'x_forwarded_for', 'x_real_ip', 'cloud_flare'),
        $v4_only = true
    ) {
        ...
        if ( isset($_GET['sfw_test_ip']) ) {
            if ( Helper::ipValidate($_GET['sfw_test_ip']) !== false ) {
            ...
            if ( !empty($result->row) ) {
                if ( 1 == $result->row['status'] ) {
                    ...
                    $this->blocked_ips[$origin] = array(
                        ...
                        'mask' => Helper::ipMaskLongToNumber($result->row['mask']),
                    );
                    $this->all_ips[$origin] = array(
                        'ip' => $current_ip,
                        'network' => long2ip($result->row['network']),
     ...
     */
    public function logsUpdate($ip, $result)
    {
     ...
     */
    public function logsSend($ct_key)
    {
        //Getting logs
    ...
    public function sfwUpdate($api_key, $immediate = false)
    {
        if ( !empty($api_key) ) {
            ...
            if ( !isset($api_server, $data_id, $file_url_nums) ) {
                $result = $this->getSfwFile($api_key, $immediate);

                return !empty($result['error'])
                    ? $result
                    : true;
            } elseif ( $api_server && $data_id && is_array($file_url_nums) && count($file_url_nums) ) {
                $result = $this->sfwUpdateDb($api_server, $data_id, $file_url_nums[0]);
                    ...
                    if ( count($file_url_nums) ) {
                        Helper::httpRequest(
                            ...
                            array(
                                ...
                                'spbc_remote_call_action' => 'sfwUpdate',
                                'plugin_name' => 'apbct',
    ...
    private function getSfwFile($api_key, $immediate)
    {
        $result = API::methodGet2sBlacklistsDb($api_key, 'multifiles', '3_2');

        if ( empty($result['error']) ) {
            if ( !empty($result['file_url']) ) {
                if ( Helper::httpRequest($result['file_url'], array(), 'get_code') === 200 ) {
                    if ( ini_get('allow_url_fopen') ) {
                        ...
                        $truncate_check = $this->db->query(
                            "SELECT COUNT(network) as cnt FROM {$this->data_table};"
                        ...
                        if ( preg_match('/multifiles/', $result['file_url']) ) {
                            $api_server = preg_replace(
                                '@https://(api.*?)\.cleantalk\.org/.*?(bl_list_[0-9a-z]*?)\.multifiles\.csv\.gz@',
                                '$1',
                                $result['file_url']
                            );
                            $data_id = preg_replace(
                                '@https://(api.*?)\.cleantalk\.org/.*?(bl_list_[0-9a-z]*?)\.multifiles\.csv\.gz@',
                                '$2',
                                $result['file_url']
                            );
    ...
    private function sfwUpdateDb($api_server = null, $data_id = null, $file_url_num = null)
    {
        $file_url = 'https://' . $api_server . '.cleantalk.org/store/' . $data_id . '.' . $file_url_num . '.csv.gz';

        if ( Helper::httpRequest($file_url, array(), 'get_code') === 200 ) { // Check if it's there
     ...
     * @param bool $test
     */
    public function sfwDie($api_key, $cookie_prefix = '', $cookie_domain = '', $test = false)
    {
        // Headers
        ...
        if ( file_exists(__DIR__ . DIRECTORY_SEPARATOR . "sfw_die_page.html") ) {
            ...
            $sfw_die_page = str_replace(
                '{TEST_TITLE}',
                ($this->test ? __('This is the testing page for SpamFireWall', 'cleantalk-spam-protect') : ''),
                $sfw_die_page
            );
            ...
            if ( $this->test ) {
                ...
                $sfw_die_page = str_replace(
                    '{TEST_IP_BLOCKED}',
                    $this->all_ips['sfw_test']['status'] == 1 ? 'Passed' : 'Blocked',
                    $sfw_die_page
                );
                $sfw_die_page = str_replace(
                    '{REAL_IP_BLOCKED}',
                    $this->all_ips['real']['status'] == 1 ? 'Passed' : 'Blocked',
                    $sfw_die_page
                );
            } else {
            ...
            $sfw_die_page = str_replace(
                '{REMOTE_ADDRESS}',
                $this->blocked_ips ? $this->blocked_ips[key($this->blocked_ips)]['ip'] : '',
                $sfw_die_page
            );
            ...
            $sfw_die_page = str_replace(
                '{GENERATED}',
                "<p>The page was generated at&nbsp;" . date("D, d M Y H:i:s") . "</p>",
                $sfw_die_page
            );
