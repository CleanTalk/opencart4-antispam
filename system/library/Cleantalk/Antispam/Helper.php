...
class Helper
{
    ...
    public static function ipGet(
        $ip_types = array('real', 'remote_addr', 'x_forwarded_for', 'x_real_ip', 'cloud_flare'),
        $v4_only = true
    ) {
        ...
        if ( isset($ips['remote_addr']) ) {
            $ip_type = self::ipValidate(isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '');
            if ( $ip_type ) {
                $ips['remote_addr'] = ($ip_type == 'v6' ? self::ipV6Normalize(
        ...
        if ( isset($ips['x_forwarded_for']) ) {
            if ( isset($headers['X-Forwarded-For']) ) {
                ...
                $tmp = trim($tmp[0]);
                $ip_type = self::ipValidate($tmp);
                if ( $ip_type ) {
                    $ips['x_forwarded_for'] = $ip_type == 'v6' ? self::ipV6Normalize($tmp) : $tmp;
                }
            }
        }
        ...
        if ( isset($ips['cloud_flare']) ) {
            if ( isset($headers['CF-Connecting-IP'], $headers['CF-IPCountry'], $headers['CF-RAY']) || isset($headers['Cf-Connecting-Ip'], $headers['Cf-Ipcountry'], $headers['Cf-Ray']) ) {
                ...
                $ip_type = self::ipValidate(trim($tmp[0]));
                if ( $ip_type ) {
                    $ips['real'] = $ip_type == 'v6' ? self::ipV6Normalize(trim($tmp[0])) : trim($tmp[0]);
                }
            }
        }
        ...
        if ( isset($ips['real']) ) {
            ...
            $ip_type = self::ipValidate(isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '');
            if ( $ip_type ) {
                $ips['real'] = ($ip_type == 'v6' ? self::ipV6Normalize(
                    isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : ''
                ) : isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '';
            }
            ...
            if ( isset($headers['CF-Connecting-IP'], $headers['CF-IPCountry'], $headers['CF-RAY']) || isset($headers['Cf-Connecting-Ip'], $headers['Cf-Ipcountry'], $headers['Cf-Ray']) ) {
                ...
                $ip_type = self::ipValidate($headers['X-Sucuri-Clientip']);
                if ( $ip_type ) {
                    $ips['real'] = $ip_type == 'v6' ? self::ipV6Normalize(
                ...
                $ip_type = self::ipValidate($headers['X-Cdn-Any-Ip']);
                if ( $ip_type ) {
                    $ips['real'] = $ip_type == 'v6' ? self::ipV6Normalize(
                ...
                $ip_type = self::ipValidate($headers['Incap-Client-Ip']);
                if ( $ip_type ) {
                    $ips['real'] = $ip_type == 'v6' ? self::ipV6Normalize(
            ...
            if ( $ip_type === false ||
                ($ip_type &&
                    (self::ipIsPrivateNetwork($ips['real'], $ip_type) ||
                        self::ipMaskMatch(
                            $ips['real'],
                ...
                if ( isset($headers['X-Forwarded-For']) ) {
                    ...
                    $ip_type = self::ipValidate($tmp);
                    if ( $ip_type ) {
                        $ips['real'] = $ip_type == 'v6' ? self::ipV6Normalize($tmp) : $tmp;
                    }
                    // X-Real-Ip
    ...
    public static function ipMaskMatch($ip, $cidr, $ip_type = 'v4', $xtet_count = 0)
    {
        ...
        $mask = min($mask - $xtet_base * $xtet_count, $xtet_base);
        ...
        $net_part = isset($net_ip_xtets[$xtet_count]) ? $net_ip_xtets[$xtet_count] : '';
        $ip_part  = isset($ip_xtets[$xtet_count]) ? $ip_xtets[$xtet_count] : '';

        if ($ip_type === 'v4') {
            $net_val = (int)$net_part;
            $ip_val  = (int)$ip_part;
        } else {
            // Validate hex to avoid PHP warning in hexdec()
            if (!preg_match('/^[0-9a-f]{1,4}$/i', $net_part) || !preg_match('/^[0-9a-f]{1,4}$/i', $ip_part)) {
                return false;
            }

            $net_val = hexdec($net_part);
            $ip_val  = hexdec($ip_part);
        }

        $net_xtet = str_pad(decbin($net_val), $xtet_base, 0, STR_PAD_LEFT);
        $ip_xtet  = str_pad(decbin($ip_val), $xtet_base, 0, STR_PAD_LEFT);
     ...
     * @return void
     */
    public static function apbctCookieSet(
        $name,
        $value = '',
        $expires = 0,
        $path = '',
